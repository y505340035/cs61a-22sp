...
def about(topic):
    ...
    def has_topic(paragraph):
        ...
        para_word = split(paragraph)
        # print('DEBUG:',paragraph)
        for s in para_word:
            # print('DEBUG:','----')
            # print('DEBUG:','para_word:', s, len(s))
            for tpc in topic:
                # print('DEBUG:','tpc:', tpc, len(tpc))
                if s == tpc:
                    return True
        return False
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for w in word_list:
        ...
        if diff < diff_min or (diff == diff_min and res == typed_word):
            diff_min = diff
            res = w
...
def minimum_mewtations(start, goal, limit):
    ...
    diff = start[0] != goal[0]
    add = 1 + minimum_mewtations(start[1:], goal, limit-1)
    remove = 1 + minimum_mewtations(start, goal[1:], limit-1)
