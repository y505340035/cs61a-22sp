...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        distan = 0         
        aim = None
        while target_place != None and not(target_place.is_hive) and (distan <= self.max_range or self.max_range == 0):
            if distan >= self.min_range:
                aim = random_bee(target_place.bees)
                if aim != None:
                    break
            target_place = target_place.entrance
            distan += 1
        return aim  # REPLACE THIS LINE
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        self.course_damage(amount)
        if self.health <= amount:
            self.course_damage(self.damage)            
        return super().reduce_health(amount)
    
    def course_damage(self, amount):
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.chew_timer == 0:
            ...
            self.chew_timer += 1
        if self.chew_timer == self.time_to_chew + 1:
            self.chew_timer = 0
    ...
    def action(self, gamestate):
        ...
        while place != None:
            if place.ant == None:
                place = place.exit
                continue
            self.double(place.ant)
            if place.ant.is_container and place.ant.ant_contained != None:
                self.double(place.ant.ant_contained)
            place = place.exit

    def double(self, bee):
        if not(bee in self.enhanced_bees):
            self.enhanced_bees.append(bee)
            bee.damage = bee.damage * 2
        # END Problem 12
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.scared_timer > 0:
            ...
            if self.place.ant != None or entrance.is_hive:
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if target and target.has_scared == False:
            target.scare(2)
